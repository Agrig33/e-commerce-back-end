...
router.put('/:id', (req, res) => {
  ...
  .then((tag) => {
    if (req.body.productIds && req.body.productIds.lenght) {
      ProductTag.findAll({
        where: { tag_id: req.params.id }
      }).then((tag) => {
        const tagIds = tag.map(({ product_id }) => product_id);
        const newTagIds = req.body.prouductIds
        ...
        const tagIdsToRemove =tag.filter(({ product_id }) => !req.body.productIds.includes(product_id))
        .map(({ id }) => id);
        return Promise.all([
